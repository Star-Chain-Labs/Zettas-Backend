...
function generateRandomRoiTillTarget(
  ...
  // Average daily ROI required
  const avgDaily = remainingRoi / remainingDays;

  // Random factor 0.5x - 1.3x to simulate bigger fluctuations
  ...
  try {
    ...
    if (isNaN(tradeAmount) || tradeAmount <= 0) {
      return res.status(200).json({
        success: false,
        message:
          "No trade executed today due to unfavorable market conditions. Please check back tomorrow.",
        tradeStatus: "skipped",
      });
    }
