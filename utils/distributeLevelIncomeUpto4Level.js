...
export const distributeCommissions4Level = async (user, investedAmount) => {
  try {
    ...
    for (let i = 0; i < uplineChain.length; i++) {
      ...
      const directReferralsCount = await UserModel.countDocuments({
        sponsorId: uplineUser._id,
      });
      const allowedLevels =
        directReferralsCount >= 3 ? 5 : directReferralsCount >= 2 ? 2 : 0;
      if (level > allowedLevels) {
        console.log(
          `⛔ Skipping level ${level} income for ${uplineUser.username} (has only ${directReferralsCount} referrals)`
        );
        continue;
      }
      if (uplineUser.isIncomeBlocked) {
        console.log(
          `⚠️ Skipping commission for ${uplineUser.username} (income blocked)`
        );
        continue;
      }
      ...
      console.log(
        `✅ Level ${level} commission ₹${commissionAmount} to ${uplineUser.username}`
      );
